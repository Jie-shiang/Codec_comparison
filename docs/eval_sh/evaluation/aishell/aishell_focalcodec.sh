...
# FocalCodec Evaluation Script - Aishell Dataset
...
echo "Starting FocalCodec Evaluation - Aishell Dataset"

# FocalCodec 12.5Hz - Aishell (zh)
echo "Running: FocalCodec 12.5Hz - Aishell (zh)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/FocalCodec/12.5HZ/aishell \
...
# FocalCodec 25Hz - Aishell (zh)
echo "Running: FocalCodec 25Hz - Aishell (zh)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/FocalCodec/25HZ/aishell \
    --csv_file "${CSV_DIR}/aishell_filtered_clean.csv" \
    --original_dir /mnt/Internal/ASR \
    --model_name "FocalCodec" \
    --frequency "25Hz" \
...
# FocalCodec 50Hz - Aishell (zh)
echo "Running: FocalCodec 50Hz - Aishell (zh)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/FocalCodec/50HZ/aishell \
    --csv_file "${CSV_DIR}/aishell_filtered_clean.csv" \
    --original_dir /mnt/Internal/ASR \
    --model_name "FocalCodec" \
    --frequency "50Hz" \
...
echo "FocalCodec evaluation on Aishell completed."
