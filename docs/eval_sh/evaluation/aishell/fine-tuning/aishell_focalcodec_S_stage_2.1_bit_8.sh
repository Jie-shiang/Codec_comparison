...
echo "Starting FocalCodec-S Stage 2.1 (8-bit) Evaluation"

conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/FocalCodec-S/50Hz_2k_finetune/trained_stage_2.1_8bit \
    --csv_file "${CSV_DIR}/aishell_filtered_clean.csv" \
    --original_dir /mnt/Internal/ASR \
    --model_name "FocalCodec-S" \
    --frequency "50Hz_2k_stage_2.1_8bit" \
    --causality "Causal" \
    --bit_rate "0.40" \
    --quantizers "1" \
    --codebook_size "2048" \
