...
# Mimi Evaluation Script - Aishell Dataset
...
echo "Starting Mimi Evaluation - Aishell Dataset"

# Mimi 12.5Hz_8k - Aishell (zh)
echo "Running: Mimi 12.5Hz_8k - Aishell (zh)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/MimiCodec/12.5Hz_8k/aishell \
...
# Mimi 12.5Hz_16k - Aishell (zh)
echo "Running: Mimi 12.5Hz_16k - Aishell (zh)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/MimiCodec/12.5Hz_16k/aishell \
    --csv_file "${CSV_DIR}/aishell_filtered_clean.csv" \
    --original_dir /mnt/Internal/ASR \
    --model_name "Mimi" \
...
echo "Mimi evaluation on Aishell completed."
