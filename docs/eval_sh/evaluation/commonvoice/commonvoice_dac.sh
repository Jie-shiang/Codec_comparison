...
# DAC Evaluation Script - Commonvoice Dataset
...
echo "Starting DAC Evaluation - Commonvoice Dataset"

# DAC 50Hz_16kHz - Commonvoice (zh)
echo "Running: DAC 50Hz_16kHz - Commonvoice (zh)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/DAC/50Hz_16kHz/commonvoice \
...
# DAC 50Hz_24kHz - Commonvoice (zh)
echo "Running: DAC 50Hz_24kHz - Commonvoice (zh)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/DAC/50Hz_24kHz/commonvoice \
    --csv_file "${CSV_DIR}/commonvoice_filtered_clean.csv" \
    --original_dir /mnt/Internal/ASR \
    --model_name "DAC" \
...
# DAC 50Hz_44kHz - Commonvoice (zh)
echo "Running: DAC 50Hz_44kHz - Commonvoice (zh)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/DAC/50Hz_44kHz/commonvoice \
    --csv_file "${CSV_DIR}/commonvoice_filtered_clean.csv" \
    --original_dir /mnt/Internal/ASR \
    --model_name "DAC" \
...
echo "DAC evaluation on Commonvoice completed."
