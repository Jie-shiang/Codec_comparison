...
# LSCodec Evaluation Script - Commonvoice Dataset
...
echo "Starting LSCodec Evaluation - Commonvoice Dataset"

# LSCodec 25Hz - Commonvoice (zh)
echo "Running: LSCodec 25Hz - Commonvoice (zh)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/LSCodec/25Hz/commonvoice \
...
# LSCodec 50Hz - Commonvoice (zh)
echo "Running: LSCodec 50Hz - Commonvoice (zh)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
...
echo "LSCodec evaluation on Commonvoice completed."
