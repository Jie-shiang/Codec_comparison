...
# LSCodec Evaluation Script - LibriSpeech Dataset
...
echo "Starting LSCodec Evaluation - LibriSpeech Dataset"

# LSCodec 25Hz - LibriSpeech (English)
echo "Running: LSCodec 25Hz - LibriSpeech (English)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/LSCodec/25Hz/librispeech \
...
# LSCodec 50Hz - LibriSpeech (English)
echo "Running: LSCodec 50Hz - LibriSpeech (English)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/LSCodec/50Hz/librispeech \
    --csv_file "${CSV_DIR}/librispeech_filtered_clean.csv" \
    --original_dir /mnt/Internal/ASR \
    --model_name "LSCodec" \
    --frequency "50Hz" \
...
echo "LSCodec evaluation on LibriSpeech completed."
