...
# DAC Evaluation Script - Vivos Dataset
...
echo "Starting DAC Evaluation - Vivos Dataset"

# DAC 50Hz_16kHz - Vivos (vi)
echo "Running: DAC 50Hz_16kHz - Vivos (vi)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/DAC/50Hz_16kHz/vivos \
...
# DAC 50Hz_24kHz - Vivos (vi)
echo "Running: DAC 50Hz_24kHz - Vivos (vi)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/DAC/50Hz_24kHz/vivos \
    --csv_file "${CSV_DIR}/vivos_filtered_clean.csv" \
    --original_dir /mnt/Internal/ASR \
    --model_name "DAC" \
...
# DAC 50Hz_44kHz - Vivos (vi)
echo "Running: DAC 50Hz_44kHz - Vivos (vi)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/DAC/50Hz_44kHz/vivos \
    --csv_file "${CSV_DIR}/vivos_filtered_clean.csv" \
    --original_dir /mnt/Internal/ASR \
    --model_name "DAC" \
...
echo "DAC evaluation on Vivos completed."
