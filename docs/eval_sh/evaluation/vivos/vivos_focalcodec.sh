...
# FocalCodec Evaluation Script - Vivos Dataset
...
echo "Starting FocalCodec Evaluation - Vivos Dataset"

# FocalCodec 12.5Hz - Vivos (vi)
echo "Running: FocalCodec 12.5Hz - Vivos (vi)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/FocalCodec/12.5HZ/vivos \
...
# FocalCodec 25Hz - Vivos (vi)
echo "Running: FocalCodec 25Hz - Vivos (vi)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/FocalCodec/25HZ/vivos \
    --csv_file "${CSV_DIR}/vivos_filtered_clean_test.csv" \
    --original_dir /mnt/Internal/ASR \
    --model_name "FocalCodec" \
    --frequency "25Hz" \
...
# FocalCodec 50Hz - Vivos (vi)
echo "Running: FocalCodec 50Hz - Vivos (vi)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/FocalCodec/50HZ/vivos \
    --csv_file "${CSV_DIR}/vivos_filtered_clean_test.csv" \
    --original_dir /mnt/Internal/ASR \
    --model_name "FocalCodec" \
    --frequency "50Hz" \
...
echo "FocalCodec evaluation on Vivos completed."
