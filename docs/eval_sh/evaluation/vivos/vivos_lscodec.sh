...
# LSCodec Evaluation Script - Vivos Dataset
...
echo "Starting LSCodec Evaluation - Vivos Dataset"

# LSCodec 25Hz - Vivos (vi)
echo "Running: LSCodec 25Hz - Vivos (vi)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/LSCodec/25Hz/vivos \
...
# LSCodec 50Hz - Vivos (vi)
echo "Running: LSCodec 50Hz - Vivos (vi)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/LSCodec/50Hz/vivos \
    --csv_file "${CSV_DIR}/vivos_filtered_clean_test.csv" \
    --original_dir /mnt/Internal/ASR \
    --model_name "LSCodec" \
    --frequency "50Hz" \
...
echo "LSCodec evaluation on Vivos completed."
