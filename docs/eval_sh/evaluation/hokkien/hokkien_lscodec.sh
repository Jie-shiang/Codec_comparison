...
# LSCodec Evaluation Script - Hokkien Dataset
...
echo "Starting LSCodec Evaluation - Hokkien Dataset"

# LSCodec 25Hz - Hokkien (min)
echo "Running: LSCodec 25Hz - Hokkien (min)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/LSCodec/25Hz/hokkien \
...
# LSCodec 50Hz - Hokkien (min)
echo "Running: LSCodec 50Hz - Hokkien (min)"
conda run -n "$EVAL_ENV" python "${SCRIPT_DIR}/fast_evaluation_pipeline.py" \
    --inference_dir /mnt/Internal/jieshiang/Inference_Result/LSCodec/50Hz/hokkien \
    --csv_file "${CSV_DIR}/hokkien_filtered_clean_test.csv" \
    --original_dir /mnt/Internal/ASR \
    --model_name "LSCodec" \
    --frequency "50Hz" \
...
echo "LSCodec evaluation on Hokkien completed."
